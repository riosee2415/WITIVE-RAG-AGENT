...
- Next.js는 자체 사용량 시스템을 보유 (`00-scope.md` §2.2)하므로 본 서버의 per-tenant 메트릭은 **운영 진단용**으로만 사용 — 과금 정합성과 무관.
...
| `bedrock_input_tokens_total` (테넌트당) | 일일 임계 초과 | Next.js 사용량 시스템과 cross check (과금 정합성) |
| 동일 사용자 5분 200건+ | 비정상 | 로그·Next.js 알림. 본 서버는 차단하지 않음 (Next.js 책임) |

본 서버는 **사용량 한도 차단을 직접 하지 않는다**. 한도·차단·과금은 Next.js 책임 (`00-scope.md`). 본 서버는 메트릭만 노출.
